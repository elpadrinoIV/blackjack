...
class TestPagoApuestas < Test::Unit::TestCase
  def setup
    ...
    dinero_que_deberia_tener = @dinero_inicial + @dinero_apuesta
		...
		assert_equal(dinero_que_deberia_tener, @jugador.get_dinero, "El jugador ganó y debería haber cobrado")
    # me fijo que no quede plata en las apuestas

    @jugador.get_apuestas.each{ |apuesta|
      assert_equal(0, apuesta, "Después de terminar la mano no debería haber dinero en las apuestas")
    }
	end
  ...
  def test_jugador_pierde_comun
    ...
    dinero_que_deberia_tener = @dinero_inicial - @dinero_apuesta
		...
		assert_equal(dinero_que_deberia_tener, @jugador.get_dinero, "El jugador perdió y no debería haber cobrado")
    assert_equal(@dinero_apuesta, dinero_cobrado, "La banca debería haber cobrado porque perdió el jugador")
    # me fijo que no quede plata en las apuestas
    @jugador.get_apuestas.each{ |apuesta|
      assert_equal(0, apuesta, "Después de terminar la mano no debería haber dinero en las apuestas")
    }
  ...
  def test_jugador_duplica_y_pierde
    ...
    dinero_que_deberia_tener = @dinero_inicial - @dinero_apuesta*2
		...
		assert_equal(dinero_que_deberia_tener, @jugador.get_dinero, "El jugador ganó y debería haber cobrado")
    assert_equal(@dinero_apuesta*2, dinero_cobrado, "La banca debería haber cobrado porque perdió el jugador")
  ...
  def test_jugador_paga_seguro_no_blackjack_y_gana
    ...
    assert_equal(0, @jugador.get_apuesta_seguro, "Después de terminar la mano no debería haber dinero en las apuestas")
	end
